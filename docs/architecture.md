...
> tree rather than a pinned release (Go module `github.com/mhsanaei/KAYGEZ/v3`).
>
...
KAYGEZ is a **web control panel for [Xray-core](https://github.com/XTLS/Xray-core)**. The Go
...
2. **The Runtime abstraction (multi-node).** A panel can manage remote "nodes" (other KAYGEZ
  ...
  → POST {basePath}/panel/api/...
    → Gin engine (internal/web/web.go: initRouter)
      → middleware chain: SecurityHeaders → MaxBodyBytes (10 MiB; importDB exempt)
                          → [DomainValidator, if webDomain set] → gzip → sessions("Kaygez")
...
A "node" (`model.Node`) is another KAYGEZ instance this panel controls. Every state-changing
...
- **Module path is `.../v3`.** Internal imports use `github.com/mhsanaei/KAYGEZ/v3/internal/...`.
