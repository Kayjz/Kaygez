...
export default function ClientFormModal({
    ...
    if (isEdit && client) {
      ...
      const seed: Values = {
        ...
        security:
          ...
          client.security === "zero"
            ? "auto"
            : client.security,
        reverseTag: client.reverse?.tag || "",
        totalGB: bytesToGB(client.totalGB || 0),
        reset: Number(client.reset) || 0,
        usageMultiplier: Number(client.usageMultiplier) || 1.0,
        limitIp: client.limitIp || 0,
        uploadMbps: client.uploadMbps || 0,
        downloadMbps: client.downloadMbps || 0,
  ...
  const mtprotoDomain = useMemo(() => {
    for (const id of inboundIds || []) {
      ...
      if (ib?.protocol === "mtproto" && ib.mtprotoDomain)
        return ib.mtprotoDomain;
    }
  ...
  async function onSubmit() {
    ...
    const clientPayload: Record<string, unknown> = {
      ...
      uploadMbps: Number(values.uploadMbps) || 0,
      downloadMbps: Number(values.downloadMbps) || 0,
  ...
  return (
    <>
      ...
      <Modal
        open={open}
        title={
          isEdit ? t("pages.clients.editClient") : t("pages.clients.addClient")
        }
        destroyOnHidden
        width={720}
                              ...
                              {!isEdit && (
                                <Button
                                  ...
                                  onClick={() =>
                                    methods.setValue(
                                      "email",
                                      RandomUtil.randomLowerAndNum(12),
                                    )
                                  }
                                />
                              )}
                      ...
                      <Row gutter={16}>
                        <Col xs={24} md={12}>
                          <FormField
                            ...
                            label={t("pages.clients.usageMultiplier")}
                            tooltip={t("pages.clients.usageMultiplierDesc")}
                            ...
                            <Col xs={24} md={12}>
                              ...
                              >
                                <InputNumber
                                  ...
                                  controls={false}
                                  placeholder={t(
                                    "pages.clients.telegramIdPlaceholder",
                                  )}
                                  style={{ width: "100%" }}
                                />
                              </FormField>
                          ...
                          {showReverseTag && (
                            <Col xs={24} md={12}>
                              ...
                              >
                                <Input
                                  placeholder={t(
                                    "pages.clients.reverseTagPlaceholder",
                                  )}
                                />
                              </FormField>
                            </Col>
                  ...
                  children: (
                    <>
                      <Form.Item label={t("pages.clients.uuid")}>
                        <Space.Compact style={{ display: "flex" }}>
                          ...
                          <Button
                            ...
                            icon={<ReloadOutlined />}
                            onClick={() =>
                              methods.setValue("uuid", RandomUtil.randomUUID())
                            }
                          />
                        </Space.Compact>
                      </Form.Item>
                      ...
                      {showSecurity && (
                        <FormField
                          name="security"
                          label={t("pages.clients.vmessSecurity")}
                        >
                          <Select
                            options={VMESS_SECURITY_OPTIONS.map((k) => ({
                              value: k,
                              label: k,
                            }))}
                          />
                      ...
                      {showWireguard && (
                        <>
                          <Form.Item
                            label={t("pages.clients.wireguardPrivateKey")}
                          >
                            <Space.Compact style={{ display: "flex" }}>
                              <Input
                                value={wgPrivateKey}
                                ...
                                onChange={(e) => {
                                  ...
                                  methods.setValue(
                                    "wgPublicKey",
                                    priv
                                      ? Wireguard.generateKeypair(priv)
                                          .publicKey
                                      : "",
                                  );
                                }}
