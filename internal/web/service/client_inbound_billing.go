...
// parseClientInboundStatEmail resolves the inbound id from a Kaygez runtime
...
// addAccurateClientInboundTraffic consumes Kaygez runtime stat emails and
...
func (s *InboundService) addAccurateClientInboundTraffic(tx *gorm.DB, traffics []*xray.ClientTraffic) ([]*xray.ClientTraffic, error) {
	...
	for _, batch := range chunkStrings(uniqueNonEmptyStrings(statEmails), sqlInChunk) {
		...
		if err := tx.Table("client_inbound_traffics AS cit").
			Select(`cit.stat_email, cit.email, cit.inbound_id,
			        ...
			        COALESCE(NULLIF(c.usage_multiplier, 0), 1) AS client_usage_multiplier,
	...
	for _, t := range statTraffics {
		...
		multiplier := normalizeInboundUsageMultiplier(mapping.UsageMultiplier) * normalizeInboundUsageMultiplier(mapping.ClientUsageMultiplier)
