...
const (
	panelUpdaterURL      = "https://raw.githubusercontent.com/Kayjz/Kaygez/main/update.sh"
...
// GetUpdateInfo checks GitHub for the latest Kaygez release. When the dev
...
func downloadPanelUpdater() (string, error) {
	...
	file, err := os.CreateTemp("", "kaygez-update-*.sh")
	if err != nil {
		return "", err
	}
	path := file.Name()
...
func fetchPanelRelease(tag string) (*service.Release, error) {
	url := "https://api.github.com/repos/Kayjz/Kaygez/releases/latest"
	if tag != "" {
		url = "https://api.github.com/repos/Kayjz/Kaygez/releases/tags/" + tag
	}
